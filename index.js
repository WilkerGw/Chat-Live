// chat-backend/index.js - CORRIGIDO SEM ALTERAR NOMES
...
const io = new Server(server, { 
  cors: {
    origin: ["http://localhost:3000", "https://chat-live-front.vercel.app"],
    methods: ["GET", "POST"]
  }
});
...
async function seedDatabase() {
  console.log("A verificar e a popular o banco de dados com novos avatares...");
  ...
  const passwordWilker = bcrypt.hashSync('123', salt);
  const passwordJr = bcrypt.hashSync('456', salt);
  
  const wilkerAvatarUrl = "https://res.cloudinary.com/djepel3xf/image/upload/v1750356684/Captura_de_tela_2025-06-19_150935_r01z2i.png";
  const jrAvatarUrl = "https://res.cloudinary.com/djepel3xf/image/upload/v1750356683/Captura_de_tela_2025-06-19_151012_irbdin.png";

  await prisma.user.upsert({ 
    where: { username: 'wilker' }, 
    update: { avatarUrl: wilkerAvatarUrl }, 
    create: { username: 'wilker', password: passwordWilker, avatarUrl: wilkerAvatarUrl } 
  });
  await prisma.user.upsert({ 
    where: { username: 'jr' }, 
    update: { avatarUrl: jrAvatarUrl }, 
    create: { 
      username: 'jr', 
      // --- CORREÇÃO APLICADA AQUI ---
      password: passwordJr, // Corrigido de 'passwordjr' para 'passwordJr'
      avatarUrl: jrAvatarUrl 
    } 
  });
