...
const io = new Server(server, {
  cors: {
    // Agora aceitamos um array de origens
...
io.use(async (socket, next) => {
  ...
  try {
    ...
    if (!user)
      return next(new Error("Autenticação falhou: Utilizador não encontrado."));
    socket.user = user;
    next();
  } catch (err) {
...
io.on("connection", async (socket) => {
  console.log(
    `O usuário '${socket.user.username}' (ID: ${socket.id}) se conectou.`
  );
  ...
  if (defaultRoom) {
    ...
    const historyForFrontend = historyFromDb.map((msg) => ({
      ...msg,
      user: msg.author,
  ...
  socket.on("get_dm_conversations", async () => {
    try {
      ...
      const otherUserIds = dmRooms
        .map((room) => {
          const ids = room.name.split("-").slice(1);
          return ids.find((id) => parseInt(id, 10) !== userId);
        })
      ...
      if (otherUserIds.length > 0) {
        ...
        const conversations = dmRooms
          .map((room) => {
            const otherId = parseInt(
              room.name
                .split("-")
                .slice(1)
                .find((id) => parseInt(id, 10) !== userId),
              10
            );
  ...
  socket.on("start_dm", async (targetUserId) => {
    ...
    if (targetSocket) {
      if (targetSocket.currentRoom)
        targetSocket.leave(targetSocket.currentRoom.name);
  ...
  socket.on("sendMessage", async (messageText) => {
    ...
    const messagePayload = {
      ...
      socketId: socket.id,
      roomName: socket.currentRoom.name,
    };
  ...
  socket.on("typing", () => {
    if (socket.currentRoom)
      socket.broadcast
        .to(socket.currentRoom.name)
        .emit("userTyping", socket.user);
  });
  socket.on("stopTyping", () => {
    if (socket.currentRoom)
      socket.broadcast
        .to(socket.currentRoom.name)
        .emit("userStoppedTyping", socket.user);
  });
...
app.post("/login", async (req, res) => {
  ...
  if (!username || !password)
    return res
      .status(400)
      .json({ message: "Nome de utilizador e senha são obrigatórios." });
  try {
